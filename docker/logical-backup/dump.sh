...
function list_all_replica_pods_current_node {
    get_pods "labelSelector=cluster-name%3D${SCOPE},spilo-role%3Dreplica&fieldSelector=spec.nodeName%3D${CURRENT_NODENAME}" | head -n 1
}

function list_all_replica_pods_any_node {
    get_pods "labelSelector=cluster-name%3D${SCOPE},spilo-role%3Dreplica" | head -n 1
}

function get_master_pod {
    get_pods "labelSelector=cluster-name%3D${SCOPE},spilo-role%3Dmaster" | head -n 1
}
