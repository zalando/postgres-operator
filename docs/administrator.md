...
1. The `registry.opensource.zalan.do/acid/logical-backup` example image implements the backup via `pg_dumpall` and upload of compressed and encrypted results to an S3 bucket ([image source](../docker/logical-backup/Dockerfile)); the default image is the same image built with the internal CI pipeline. `pg_dumpall` requires a `superuser` access to a DB and runs on the replica when possible.  
